    ...
    public class QuestionController : Controller
    {
        ...
        public IActionResult GetHelpContent()
        {
            ...
            try
            {
                using (SQLiteConnection connection = DatabaseConnector.CreateNewConnection())
                {
                    string selectSql = "SELECT Title, Description, Category FROM HelpContent";
                    using (SQLiteCommand selectCmd = new SQLiteCommand(selectSql, connection))
                    {
                        using (var reader = selectCmd.ExecuteReader())
                        {
            ...
            {
                Console.Error.WriteLine($" Hiba a HelpContent lekérdezésekor: {ex.Message}");
                return StatusCode(500, "Hiba történt a segítség tartalom betöltésekor.");
            }
        }
