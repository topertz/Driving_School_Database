            ...
            try
            {
                using (SQLiteConnection connection = DatabaseConnector.CreateNewConnection())
                {
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand(insertSql, connection))
                    {
                        ...
                        DateTime localTime = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, TimeZoneInfo.FindSystemTimeZoneById("Central European Standard Time"));
                        string timestamp = localTime.ToString("yyyy-MM-dd HH:mm:ss");
